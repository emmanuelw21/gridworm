...
import {
  ...
  Grid, Maximize2, Pin, PinOff, AlertTriangle, RefreshCw, Boxes, Layers, MousePointerClick
} from 'lucide-react';
    ...
    >
      ...
      {isExpanded && (
        <>
          ...
          <div className='p-3 border-b border-gray-200 dark:border-gray-700'>
            ...
            {showFilters && (
              <div className='p-3 mb-1 bg-gray-50 dark:bg-gray-800 rounded-md border border-gray-200 dark:border-gray-700 flex flex-col gap-2 text-sm text-gray-700 dark:text-gray-300'>
                ...
                {['image', 'video', 'audio', '3d', 'text', 'stack'].map(category => (
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <label className="flex items-center cursor-pointer">
                        ...
                        <span className="ml-2 flex items-center capitalize">
                          ...
                          {category === 'text' && <FileText size={14} className="mr-1" />}
                          {category === 'stack' && <Layers size={14} className="mr-1" />}
                        ...
                        {(category === 'image' ? ['jpg', 'png', 'gif', 'webp', 'svg'] :
                          category === 'video' ? ['mp4', 'webm', 'mov', 'avi', 'mkv'] :
                            category === 'audio' ? ['mp3', 'wav', 'ogg', 'flac'] :
                              category === '3d' ? ['gltf', 'glb', 'obj', 'fbx', 'stl'] :
                                ['text', 'pdf']).map(ext => (
                                <button
                                  key={ext}
          ...
          <div
            ref={panelContentRef}
            className='flex-1 overflow-y-scroll p-2 pb-6 min-h-0 relative'
            style={{
      ...
      {isExpanded && (
        <div
          className="absolute top-0 right-0 bottom-0 w-2 cursor-ew-resize hover:bg-blue-300 opacity-0 hover:opacity-50"
          onMouseDown={e => {
            e.preventDefault();
            onBeginResize();
          }}
