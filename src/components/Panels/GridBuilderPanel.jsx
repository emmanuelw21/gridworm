...
import {
  ...
  Maximize, Minimize, Grid as GridIcon, Move, Filter, Target, Copy,
  MoveHorizontal, List, Trash2, Dice5, Pencil, Type, Eraser,
  RotateCw, LayoutGrid, ZoomIn, ZoomOut, Ruler,
  Image as ImageIcon, Video as VideoIcon, Boxes,
  MousePointer2 as MousePointer2Icon, Eye, EyeOff, Columns, Rows, Square, RefreshCw, FileText 
} from 'lucide-react';
import MediaPreview from '../MediaGrid/MediaPreview.jsx';
import { isVideo, isImage, isAudio, isPDF, is3D } from '../MediaGrid/helpers.js';
    ...
    <div className="flex-1 overflow-auto p-4 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100">
      ...
      <div className="flex gap-3 items-center mb-3">
        <div className="flex items-center">
          <h2 className="font-bold text-lg dark:text-gray-100">Grid Builder</h2>
          {gridHasOverflow && (
          ...
          {freeGridMode && (
            <div className="flex items-center space-x-1 p-1 border border-gray-300 dark:border-gray-600 rounded bg-gray-100 dark:bg-gray-800">
              ...
              <button
                ...
                className={`p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-700 ${eraserMode === 0 ? 'text-gray-600 dark:text-gray-300' :
        ...
        <div className="mb-4 p-3 bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-md">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <input type="range" min="1" max="40" value={gridConfig.columns} onChange={(e) => handleLocalUpdateGridConfig('columns', e.target.value)} className="w-full accent-blue-500" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1 flex items-center"><Rows size={14} className="mr-1" /> Rows: {gridConfig.rows}</label>
              <input type="range" min="1" max="40" value={gridConfig.rows} onChange={(e) => handleLocalUpdateGridConfig('rows', e.target.value)} className="w-full accent-blue-500" />
            </div>
            <div>
              ...
              <input type="range" min="60" max="500" step="10" value={gridConfig.cellWidth} onChange={(e) => handleLocalUpdateGridConfig('cellWidth', e.target.value)} className="w-full accent-blue-500" />
            </div>
            <div>
                          ...
                          {slot.isMissing ? (
                            ...
                            <>
                              <div className="flex items-center truncate">
                                ...
                                {is3D(slot.type) && <Boxes size={12} className="mr-1 flex-shrink-0" />}
                                {isPDF(slot.type) && <FileText size={12} className="mr-1 flex-shrink-0" />}
