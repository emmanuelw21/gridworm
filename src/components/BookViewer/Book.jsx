...
const preloadTexture = async (media, mediaFiles) => {
  ...
  // Include thumbnail in cache key so textures update when thumbnails change
  let cacheKey;
  if (typeof media === 'object') {
    const thumbHash = media.thumbnail ? media.thumbnail.slice(-10) : 'no-thumb';
    cacheKey = `media-${media.id}-${thumbHash}`;
  } else {
    cacheKey = `string-${media}`;
  }
...
const Page = ({ number, front, back, page, opened, bookClosed, mediaFiles, pageWidth, pageHeight, totalPages }) => {
  ...
  const manualSkinnedMesh = useMemo(() => {
    ...
    if (!picture || !picture2 || !pageGeometry || !segmentWidth) return null;
  ...
  useFrame((_, delta) => {
    ...
    // Use the validated bones from above
  ...
  // Cleanup old mesh when component unmounts or dependencies change
  useEffect(() => {
    const currentMesh = skinnedMeshRef.current;
    return () => {
      if (currentMesh) {
        // Unbind skeleton before disposal
        if (currentMesh.skeleton) {
          currentMesh.skeleton.dispose();
        }
    ...
    >
      {manualSkinnedMesh && (
        <primitive
          ...
          ref={(ref) => {
            if (ref !== skinnedMeshRef.current) {
              skinnedMeshRef.current = ref;
            }
          }}
