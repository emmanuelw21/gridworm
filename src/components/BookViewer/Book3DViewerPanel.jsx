...
import TextPageCreator from '../FreeGrid/TextPageCreator';
  ...
  return (
    <div className="bg-white/95 dark:bg-gray-800/95 backdrop-blur rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 w-64 max-h-[calc(100vh-12rem)] overflow-y-auto">
      ...
      <div className="p-4 space-y-4">
        ...
        <div>
          <label className="text-sm font-medium mb-1 block dark:text-gray-300">
            Page Thickness: {(bookConfig.pageDepth * 100).toFixed(1)}
          </label>
          <input
            type="range"
            min="0.001"
            max="0.01"
            step="0.0001"
            value={bookConfig.pageDepth}
            onChange={(e) => setBookConfig(prev => ({ ...prev, pageDepth: parseFloat(e.target.value) }))}
            className="w-full cursor-pointer"
          />
        </div>
...
const PageThumbnails = memo(({
  ...
  const activeThumbIndex = thumbnailData.findIndex(thumb => thumb.pageIndex === currentPage);
  ...
  return (
    <div className={`absolute bottom-4 left-4 right-4 transition-all duration-300 ease-in-out ${isDocked ? 'transform translate-y-[calc(100%-3rem)]' : 'transform translate-y-0'
      ...
      <div className="absolute -bottom-10 left-1/2 transform -translate-x-1/2 text-center text-sm text-white bg-black/50 px-3 py-1 rounded-full backdrop-blur">
        Page {currentPage + 1} of {atomPages.length}
      </div>
    </div>
  );
});
...
const Book3DViewerPanel = ({
  ...
  const generatePagesFromGrid = useCallback((mapping) => {
    const sourceItems = freeGridItems && Object.keys(freeGridItems).length > 0
      ? Object.values(freeGridItems).map(item => {
        const media = mediaFiles.find(m => m.id === item.mediaId);
        return media || null;
      }).filter(Boolean)
      : gridSlots.filter(Boolean).map(slot => {
    ...
    return pages;
  }, [freeGridItems, gridSlots, mediaFiles]);

  // Handle page changes
  const handlePageChange = useCallback((newPage) => {
    const validPage = Math.max(0, Math.min(newPage, atomPages.length - 1));
    setCurrentPage(validPage);
  }, [atomPages.length, setCurrentPage]);

  // Handle thumbnail click
  const handleThumbnailClick = useCallback((targetPage) => {
    const validPage = Math.max(0, Math.min(targetPage, atomPages.length - 1));
    setCurrentPage(validPage);
  }, [atomPages.length, setCurrentPage]);
     ...
     <div className={`bg-white dark:bg-gray-900 rounded-lg shadow-2xl flex flex-col ${isMaximized ? 'w-full h-full' : 'w-11/12 h-5/6 max-w-7xl'
         ...
         <div className="flex items-center space-x-2">
           ...
           {atomPages.length > 0 && (
             <>
               ...
               <button
                 onClick={() => handlePageChange(currentPage + 1)}
                 disabled={currentPage >= atomPages.length - 1}
                 className="p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                 title="Next Page"
               >
       ...
       {showTextCreator && (
         <TextPageCreator
           ...
           onCreatePages={handleCreateTextPages}
           darkMode={darkMode}
           pageConfig={{
             width: 400,
             height: 600,
