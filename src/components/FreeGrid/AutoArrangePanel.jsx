...
import { 
  ...
  AlignHorizontalJustifyCenter, AlignVerticalJustifyCenter, X
} from 'lucide-react';

const AutoArrangePanel = ({ items, onArrange, darkMode, onClose }) => {
  ...
  const applyArrangement = useCallback(() => {
    ...
    // If scaleToFit is disabled, use original item sizes
    const finalArrangement = arrangedItems.map(item => {
      if (!config.scaleToFit) {
        // Find the original item to get its original dimensions
        const originalItem = items.find(i => i.id === (item.originalId || item.id));
        return {
          id: item.originalId || item.id,
          x: item.previewX,
          y: item.previewY,
          width: originalItem ? originalItem.width : item.previewWidth,
          height: originalItem ? originalItem.height : item.previewHeight,
          rotation: 0
        };
      } else {
        // Use the arranged (scaled) dimensions
        return {
          id: item.originalId || item.id,
          x: item.previewX,
          y: item.previewY,
  ...
  }, [arrangedItems, config, items, onArrange]);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 w-full max-w-3xl">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center space-x-2">
            <button
              ...
              className={`px-3 py-1 rounded text-sm ${marqueeActive ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300'} hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors`}
            >
            ...
            <button
              ...
              className="px-3 py-1 bg-gray-600 dark:bg-gray-700 text-white rounded hover:bg-gray-700 dark:hover:bg-gray-600 text-sm transition-colors"
            >
            ...
            <button
              ...
              className="px-3 py-1 bg-green-600 dark:bg-green-700 text-white rounded hover:bg-green-700 dark:hover:bg-green-600 text-sm transition-colors"
            >
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-gray-700 dark:text-gray-300">{Math.round(previewScale * 100)}%</span>
            <button
          ...
          <div className="flex items-center space-x-2">
            ...
            {aspectRatios.map(ratio => (
              <button
                ...
                className={`p-1.5 rounded ${aspectRatio === ratio.value ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300'} hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors`}
                title={ratio.label}
              >
                {ratio.icon}
              </button>
        ...
        <div className="flex items-center space-x-4">
          <label className="flex items-center space-x-2">
            <input
              ...
              className="rounded text-blue-600 dark:text-blue-400 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400"
            />
            <span className="text-sm text-gray-700 dark:text-gray-300">Scale to fit</span>
          </label>
          
          <div className="flex items-center space-x-2">
            <span className="text-sm text-gray-700 dark:text-gray-300">Padding:</span>
            <input
              type="number"
              value={config.padding}
              onChange={(e) => setConfig({...config, padding: parseInt(e.target.value) || 0})}
              className="w-16 px-2 py-1 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              min="0"
              max="50"
            />
          </div>
        ...
        <div 
          ref={previewRef}
          className="relative bg-gray-100 dark:bg-gray-900 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700"
          style={{ height: '500px' }}
        >
          <div
          ...
          >
            ...
            {marqueeActive && (
              <div
                className="absolute border-2 border-blue-500 dark:border-blue-400 bg-blue-500 dark:bg-blue-400 bg-opacity-10 dark:bg-opacity-20"
                style={{
              ...
              >
                {/* Resize handles */}
                <div
                  className="absolute -top-2 -left-2 w-4 h-4 bg-blue-500 dark:bg-blue-400 rounded-full cursor-nw-resize hover:bg-blue-600 dark:hover:bg-blue-300 transition-colors"
                  onMouseDown={(e) => handleMarqueeMouseDown(e, 'tl')}
                />
                <div
                  className="absolute -top-2 -right-2 w-4 h-4 bg-blue-500 dark:bg-blue-400 rounded-full cursor-ne-resize hover:bg-blue-600 dark:hover:bg-blue-300 transition-colors"
                  onMouseDown={(e) => handleMarqueeMouseDown(e, 'tr')}
                />
                <div
                  className="absolute -bottom-2 -left-2 w-4 h-4 bg-blue-500 dark:bg-blue-400 rounded-full cursor-sw-resize hover:bg-blue-600 dark:hover:bg-blue-300 transition-colors"
                  onMouseDown={(e) => handleMarqueeMouseDown(e, 'bl')}
                />
                <div
                  className="absolute -bottom-2 -right-2 w-4 h-4 bg-blue-500 dark:bg-blue-400 rounded-full cursor-se-resize hover:bg-blue-600 dark:hover:bg-blue-300 transition-colors"
            ...
            {arrangedItems.map((item, index) => (
              <div
                ...
                className={`absolute bg-gray-300 dark:bg-gray-700 border-2 border-gray-400 dark:border-gray-600 rounded cursor-move hover:border-blue-400 dark:hover:border-blue-300 transition-all hover:shadow-md ${
