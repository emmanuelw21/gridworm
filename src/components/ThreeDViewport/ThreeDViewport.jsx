...
const ThreeDViewport = ({
  ...
  const handleThumbnailCapture = (dataURL, cameraSettings) => {
    if (thumbnailGeneratorObject) {
      ...
      // Update object with thumbnail
      setObjects(prev => prev.map(obj =>
        obj.id === objId
          ? { ...obj, thumbnail: dataURL, thumbnailSettings: cameraSettings }
          : obj
      ));
  ...
  const updateGizmoPosition = () => {
    if (!customGizmoRef.current) return;

    if (selectedObjectsRef.current.length === 1) {
      customGizmoRef.current.position.copy(selectedObjectsRef.current[0].position);
      customGizmoRef.current.visible = true;
    } else if (selectedObjectsRef.current.length > 1) {
      const center = getSelectionCenter();
      customGizmoRef.current.position.copy(center);
      customGizmoRef.current.visible = true;
    } else {
      customGizmoRef.current.visible = false;
    }
  };
  ...
  const handleGizmoInteraction = useCallback((event) => {
    if (!customGizmoRef.current || !raycasterRef.current) return;
    ...
    if (event.type === 'mousemove' && !isDraggingGizmo.current) {
      ...
      if (intersects.length > 0) {
        ...
        customGizmoRef.current.setHover(axis);
        mountRef.current.style.cursor = 'pointer';
      } else {
        customGizmoRef.current.clearHover();
    ...
    if (transformMode === 'translate') {
      ...
      if (selectedObjectsRef.current.length === 1) {
        ...
        if (axis === 'x' && !axisLocks.rotation.x) {
          obj.rotation.x = dragStartRotation.current.x + angle;
        } else if (axis === 'y' && !axisLocks.rotation.y) {
          obj.rotation.y = dragStartRotation.current.y + angle;
        } else if (axis === 'z' && !axisLocks.rotation.z) {
          obj.rotation.z = dragStartRotation.current.z + angle;
        }
      ...
      // Scale logic
      const delta = mouseRef.current.x - 0;
  ...
  const restoreSceneFromPersistentData = useCallback(() => {
    ...
    // Restore lights
    persistentSceneData.current.lights.forEach(lightData => {
      const existingLight = sceneRef.current.getObjectByName(lightData.id);
      if (!existingLight) {
        initializeLighting();
      }
    });
  ...
  useEffect(() => {
    ...
    if (persistentSceneData.current.objects.size > 0) {
    ...
    return () => {
      console.log('Cleaning up 3D viewport...');
      ...
      if (customGizmoRef.current) {
        ...
        scene.remove(customGizmoRef.current);
      }
  ...
  // Update gizmo when transform mode changes
  useEffect(() => {
    if (!sceneRef.current) return;
    ...
    if (customGizmoRef.current) {
      customGizmoRef.current.dispose();
      sceneRef.current.remove(customGizmoRef.current);
    }
    ...
    switch (transformMode) {
      case 'translate':
        newGizmo = new TranslateGizmo();
        break;
      case 'rotate':
        newGizmo = new RotateGizmo();
        break;
      case 'scale':
        newGizmo = new ScaleGizmo();
        break;
    }

    if (newGizmo) {
      newGizmo.visible = selectedObjectsRef.current.length > 0;
      sceneRef.current.add(newGizmo);
      customGizmoRef.current = newGizmo;
      updateGizmoPosition();
    }
  }, [transformMode]);
    ...
    <div className="flex flex-col h-full bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200">
      ...
      <div className="flex items-center justify-between p-2 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center space-x-2">
          <h3 className="font-semibold text-gray-700 dark:text-gray-300">3D Viewport</h3>
        ...
        <div className="flex items-center space-x-1">
          ...
          {selectedObjects.length > 1 && (
            <div className="flex items-center space-x-1 border-r pr-2 mr-2 text-gray-600 dark:text-gray-400">
              <button
          ...
          {selectedObjects.length > 0 && (
            <button
              ...
              className={`p-1.5 rounded ${showLookAtTool ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-100'}`}
              title="Look At Tool"
            >
              <Eye size={16} />
            </button>
          ...
          <button
            ...
            className={`p-1.5 rounded ${showAxes ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-100'}`}
            title="Toggle Axes"
          >
            <Box size={16} />
          </button>
          ...
          <button
            ...
            className={`p-1.5 rounded ${performanceMode ? 'bg-yellow-100 text-yellow-600' : 'hover:bg-gray-100'}`}
          ...
          <button
            ...
            className={`p-1.5 rounded ${showAutoArrange ? 'bg-green-100 text-green-600' : 'hover:bg-gray-100'}`}
            title="Auto Arrange"
          >
            <Shuffle size={16} />
          </button>
          <button
            ...
            className={`p-1.5 rounded ${showLightingPanel ? 'bg-yellow-100 text-yellow-600' : 'hover:bg-gray-100'}`}
          ...
          <button
            ...
            className={`p-1.5 rounded ${showLayersPanel ? 'bg-purple-100 text-purple-600' : 'hover:bg-gray-100'}`}
            title="Layers Panel"
          >
            <Layers size={16} />
          </button>
          <button
            ...
            className={`p-1.5 rounded ${showResourceMonitor ? 'bg-red-100 text-red-600' : 'hover:bg-gray-100'}`}
          ...
          <button
            ...
            className={`p-1.5 rounded ${showHelp ? 'bg-gray-100 text-gray-600' : 'hover:bg-gray-100'}`}
            title="Help (H)"
          >
            <HelpCircle size={16} />
          </button>
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 p-2">
        <div className="flex items-center justify-between mb-2">
          <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">Objects</h4>
          <div className="flex items-center space-x-2">
            <button
              onClick={() => {
                if (selectedObjects.length === 1) {
