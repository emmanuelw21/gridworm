...
export class MediaFile {
  ...
  getFileTypeCategory() {
    ...
    if (this.isTextPage || this.metadata?.isTextPage) {
      return 'text';
    }
    
    // Check for extracted pages
    if (this.isExtractedPage || this.metadata?.extractedFrom) {
      // If it has text content, it's a text page
      if (this.metadata?.textContent || this.metadata?.hasText) {
        return 'text';
      }
    ...
    const extension = this.name.toLowerCase().split('.').pop();
