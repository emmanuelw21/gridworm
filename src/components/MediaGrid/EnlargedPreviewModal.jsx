...
const EnlargedPreviewModal = ({
  ...
  const handleGoPrev = useCallback(() => {
    ...
    setCurrentIndex(newIndex);
    setPreviewMedia(indexedMediaList[newIndex].media);
  }, [currentIndex, indexedMediaList, repeatPlaylistActive, setPreviewMedia]);
  ...
  }, [showEnlargedPreview, handleGoPrev, handleGoNext, onClose]);
  ...
  useEffect(() => {
    const handleWheel = (e) => {
      ...
      // Use a threshold to prevent too sensitive scrolling
  ...
  }, [showEnlargedPreview, handleGoPrev, handleGoNext]);
           ...
           {is3DMedia ? (
             ...
             <img 
               ...
               onError={() => { 
                setIsLoading(false); 
                setImageError(true); 
                if (onMediaMissing && previewMedia && previewMedia.id) {
                  onMediaMissing(previewMedia.id, previewMedia.name);
                }
              }} 
             />
         ...
         <div className={`max-w-screen-xl mx-auto overflow-y-auto `}>
           <div className="relative">
             <div className="absolute top-3 right-3 z-20 flex space-x-2">
               {/*
               <button onClick={(e) => { e.stopPropagation(); onEditMetadata(); }} className="px-3 py-1.5 bg-blue-600 text-white rounded hover:bg-blue-700 flex items-center" title="Edit Metadata">
                 <Edit size={16} className="mr-1" /> Edit Metadata
               </button>
               */}
               {is3DMedia && (
                 <button
                   onClick={(e) => { e.stopPropagation(); if (onViewIn3D) onViewIn3D(previewMedia); }}
                   className="px-3 py-1.5 bg-purple-600 text-white rounded hover:bg-purple-700 flex items-center"
                 >
