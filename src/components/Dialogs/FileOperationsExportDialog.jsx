...
const FileOperationsExportDialog = ({
  ...
  const generateRenameCommands = () => {
    ...
    if (platform === 'windows_ps') {
      ...
      validItems.forEach((item, index) => {
        const originalPath = item.userPath || item.name;
        ...
        scriptLines.push(
          `# File ${index + 1}: ${originalName}`,
          `if (Test-Path -Path ${normOriginalPath} -PathType Leaf) {`,
          `    Rename-Item -Path ${normOriginalPath} -NewName '${newName}' -Force`,
          `    Write-Host "Renamed: ${originalName} -> ${newName}" -ForegroundColor Green`,
          `} else {`,
          `    Write-Host "Not found: ${normOriginalPath}" -ForegroundColor Yellow`,
          `}`,
          ``
        );
      });
      ...
      validItems.forEach(item => {
        ...
        scriptLines.push(`if exist ${normOriginalPath} ( ren ${normOriginalPath} "${newName}" )`);
      });
    } else {
      scriptLines.push(
        `#!/bin/bash`,
      ...
      validItems.forEach(item => {
        ...
        const newFullPath = item.userPath
          ? `${item.userPath.substring(0, item.userPath.lastIndexOf('/') + 1)}${newName}`
          : newName;
        const normOriginalPath = normalizePathForScript(originalPath, platform);

        scriptLines.push(`echo "Renaming ${originalName} to ${newName}..."`);
        scriptLines.push(`mv -f ${normOriginalPath} '${newFullPath}'`);
      });
    }
          ...
          {filesWithoutPaths > 0 && (
            <div className="bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded-md p-3">
              <p className="text-sm text-amber-800 dark:text-amber-200">
                ...
                <strong className="block mt-1">Run the generated script from the directory containing these files.</strong>
              </p>
            </div>
          )}
